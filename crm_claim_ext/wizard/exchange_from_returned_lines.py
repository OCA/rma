...
class exchange_from_returned_lines(osv.osv_memory):
    ...
    _columns = {
        'exchange_line_ids': fields.many2many('temp.exchange.line',
                                              string='Selected '
                                              'exchange lines'),
    }
    ...
    def _get_selected_lines(self, cr, uid, context):
        ...
        returned_lines = self.pool.get(
            'return.line').browse(cr, uid, returned_line_ids)
        M2M = []
        for line in returned_lines:
            if True:  # ADD ALL LINE line.selected:
                M2M.append(self.pool.get('temp.exchange.line').
                           create(cr,
                                  uid,
                                  {
                                      'name': "none",
...
class temp_exchange_line(osv.osv_memory):
    ...
    _columns = {
        ...
        'returned_product_id': fields.many2one('product.product',
                                               'Returned product',
                                               required=True),
        'returned_product_quantity': fields.float('Returned quantity',
                                                  digits=(12, 2),
                                                  help="Quantity "
                                                  "of product returned"),
        'returned_unit_sale_price':
        fields.float('Unit sale price', digits=(12, 2),),
        'returned_prodlot_id':
        fields.many2one('stock.production.lot', 'Returned serial/Lot'),
        'replacement_product_id':
        fields.many2one('product.product',
                        'Replacement product', required=True),
        'replacement_product_quantity':
        fields.float('Replacement quantity',
                     digits=(12, 2), help="Replacement quantity"),
        'replacement_prodlot_id':
        fields.many2one('stock.production.lot', 'Replacement serial/Lot'),
    }
