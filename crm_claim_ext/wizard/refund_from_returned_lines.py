...
class refund_from_returned_lines(osv.osv_memory):
    ...
    _description = 'Wizard to create an '
    'refund for selected product return lines'
    _columns = {
        'refund_journal': fields.many2one('account.journal',
                                          'Refund journal',
                                          select=True),
    ...
    def _get_selected_lines(self, cr, uid, context):
        ...
        returned_lines = self.pool.get(
            'claim.line').browse(cr, uid, returned_line_ids)
        M2M = []
    ...
    def _get_journal(self, cr, uid, context):
        ...
        #  ,('refund_journal','=','True')
        return self.pool.get('account.journal').\
            search(cr, uid, [('type', '=', 'sale_refund')], limit=1)[0]
                ...
                if refund_line.invoice_id:
                    ...
                    raise osv.except_osv(
                        _('Error !'), _('Cannot find any'
                                        ' invoice for the return line!'))
        return {
            'name': 'Customer Refounds',
            'view_type': 'form',
            'view_mode': 'tree,form',
            'domain': "[('type', '=', '" +
            invoice_type + "'),('partner_id','='," + str(partner_id) + ")]",
