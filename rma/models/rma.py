...
class Rma(models.Model):
    ...
    def _create_procurement_group(self, values=None):
        vals = self._prepare_common_procurement_group_vals()
        if values:
            vals.update(values)
    ...
    def _prepare_reception_procurement_vals(self, group=None):
        values = self._prepare_common_procurement_vals(group=group)
        values.update(
            {
    ...
    def _prepare_reception_procurement_group_vals(self):
        self.ensure_one()
        return {}

    def _create_reception_procurement_group(self):
        return self._create_procurement_group(
            self._prepare_reception_procurement_group_vals()
        )
    ...
    def _prepare_reception_procurement(self):
        ...
        values = self._prepare_reception_procurement_vals(group)
    ...
    def _prepare_delivery_procurement_vals(self, scheduled_date=None):
        values = self._prepare_outgoing_procurement_vals(scheduled_date=scheduled_date)
        values.update(
            {
    ...
    def _prepare_delivery_procurement(self, scheduled_date=None, qty=None, uom=None):
        self.ensure_one()
        values = self._prepare_delivery_procurement_vals(scheduled_date)
    ...
    def _prepare_delivery_procurement_group_vals(self):
        return {}

    def _create_delivery_procurement_group(self):
        return self._create_procurement_group(
            self._prepare_delivery_procurement_group_vals()
        )
    ...
    def _prepare_replace_procurement_vals(self, warehouse=None, scheduled_date=None):
        return self._prepare_outgoing_procurement_vals(warehouse, scheduled_date)

    def _prepare_replace_procurement_group_vals(self):
        return {}

    def _create_replace_procurement_group(self):
        return self._create_procurement_group(
            self._prepare_replace_procurement_group_vals()
        )
    ...
    def _prepare_replace_procurement(
        ...
        values = self._prepare_replace_procurement_vals(warehouse, scheduled_date)
        return self._prepare_procurement(
            self.partner_shipping_id.property_stock_customer,
            values,
            product,
