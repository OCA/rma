...
class stock_warehouse(models.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        sub_locations = [
            {'name': _('RMA'), 'active': True, 'field': 'lot_rma_id'},
            {'name': _('Refurbish'), 'active': True, 'field': 'lot_refurbish_id'},
            {'name': _('Carrier Loss'), 'active': True, 'field': 'lot_carrier_loss_id'},
            {'name': _('Breakage Loss'), 'active': True, 'field': 'lot_breakage_loss_id'},
        ]
        ...
        return super(stock_warehouse, self).create(cr, uid, vals=vals, context=context)

    def create_sequences_and_picking_types(self, cr, uid, warehouse, context=None):

        super(stock_warehouse, self).create_sequences_and_picking_types(cr, uid, warehouse, context=context)

        seq_obj = self.pool.get('ir.sequence')
        picking_type_obj = self.pool.get('stock.picking.type')
        # create new sequences
        in_seq_id = seq_obj.create(cr, SUPERUSER_ID, values={'name': warehouse.name + _(' Sequence in'), 'prefix': warehouse.code + 'RMA/IN/', 'padding': 5}, context=context)
        out_seq_id = seq_obj.create(cr, SUPERUSER_ID, values={'name': warehouse.name + _(' Sequence out'), 'prefix': warehouse.code + 'RMA/OUT/', 'padding': 5}, context=context)
        int_seq_id = seq_obj.create(cr, SUPERUSER_ID, values={'name': warehouse.name + _(' Sequence internal'), 'prefix': warehouse.code + 'RMA/INT/', 'padding': 5}, context=context)
