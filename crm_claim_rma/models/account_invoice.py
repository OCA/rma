...
class AccountInvoice(models.Model):
    ...
    def _refund_cleanup_lines(self, lines):
        ...
        line_ids = [cl[1] for cl in self.env.context['claim_line_ids']]
        line_ids = self.env['claim.line'].browse(line_ids)

        new_lines = []
        for line_id in line_ids.filtered(lambda r: not r.refund_line_id):
            # For each lines replace quantity and add claim_line_id
            inv_line = line_id.invoice_line_id
            clean_line = {}
            ...
            clean_line['quantity'] = line_id.product_returned_quantity
            clean_line['claim_line_id'] = [line_id.id]
